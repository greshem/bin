...
{
	if ! grep $1 .config_check >/dev/null ;then
		echo $1 not support
	fi
}
echo $#
...
cp -f $1 .config_check
############################################
check_config CONFIG_KEXEC=y
echo PCI_ISA_MCA "############################################"
check_config CCNFIG_ISA=y
check_config CCNFIG_EISA=y
############################################
echo ip autoconfig "###############################################"
...
check_config CONFIG_SCSI_BUSLOGIC=y
...
check_config CONFIG_MSDOS_FS=y
check_config CONFIG_VFAT_FS=y
check_config CONFIG_NTFS_FS=y
rm .config_check -f
