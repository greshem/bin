...
my $bkl=deal_with_one_vcproj_file($vcproj_file);
...
########################################################################
#输入是: bakfile 文件里面的字符串片段.
#输出: 项目的名称, 库路径, include路径, cl.exe 的选项 , link.exe 的选项.
#作用: 补齐默认常用功能的一些字符串.
sub gen_dsp_tag_mid()
{

	$g_cxx=check_bkl_cxxflags_str($g_cxx);
	$g_ldflags=check_bkl_ldflags_str($g_ldflags);
	...
	print $g_rc."\n";
	$mid_str.= $g_rc."\n";
	logger("bakefile win-res 字符串是 $g_rc\n");
...
sub check_bkl_ldflags_str($)
{
	...
	if($ret_str!~/lib-path.*D.*\\use\\lib/)
	{
		$ret_str.="<lib-path>D:\\usr\\lib</lib-path>\n";
	}
	if($ret_str!~/include.*D.*\\user\\include/)
	{	
		$ret_str.=" <include>D:\\usr\\include</include>\n";
		$ret_str.="<include>D:\\usr\\include\\msvc</include>\n";
	}
...
sub check_bkl_cxxflags_str($)
{
	...
	if($ret_str!~/\/Y..*"stdafx.h"/i) #
	{
		logger("把/Yu\"stdafx.h\" 之类的东西删除掉\n");
		$ret_str=~s/\/Y..*"stdafx.h"//g;
		$ret_str=~s/\/Y..*"Stdafx.h"//g;
	}
