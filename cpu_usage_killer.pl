...
#2011_03_21_14:50:55   星期一   add by greshem, 根据 cpu_count 来制作 cpu的上限 
...
while ( 1==1 )
{
	#print "Round ".$round."\n";
	$usage=get_cpu_usage();
	if($usage > ((100-10)/$cpu_count) )
	{
		$count++;
		#print "连续".(5*$count)."秒 , 一个 cpu 的占用率> 80%\n";
		#24*5 秒 -> 2分钟. 
		#240*5 秒-> 20分钟. 
		if($count> 240)
		{
...
sub get_cpu_number()
{
	open(FILE, "/proc/cpuinfo") or die("open cpuinfo error $!\n");
	my $count=0;
	for(<FILE>)
	{
		if(/^processor/)
