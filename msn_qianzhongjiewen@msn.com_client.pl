...
sub on_message {
  ...
  my $answer;
  from_to($message,"utf8", "gb2312");
  print $friendly. " gresheme says:\n  ". $message. "\n" unless ($D == 1); 
  if ($message =~ /^reply/i) {
    ...
    #system($1);
    $answer=`$1`;
    from_to($answer,'utf8','gb2312');
    $sb->sendmsg(substr($answer,0,1024));
