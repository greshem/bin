...
def get_dir_list(path):
	...
	for root, dirs, files in os.walk(path):
		for file in files:
			...
			if (cur-mtime) < 24*60*60*7 and  abs_path.endswith("txt"): #一周的文档. .ini  .cfg 文档  .pl .py  所有的文本文档.
				filelist.append(abs_path);
			if (cur-mtime) < 24*60*60*7 and  abs_path.endswith("py"): 
				filelist.append(abs_path);
			if (cur-mtime) < 24*60*60*7 and  abs_path.endswith("pl"): 
				filelist.append(abs_path);
			if (cur-mtime) < 24*60*60*7 and  abs_path.endswith("bat"): 
				filelist.append(abs_path);
			if (cur-mtime) < 24*60*60*7 and  abs_path.endswith("sh"): 
				filelist.append(abs_path);
			if (cur-mtime) < 24*60*60*7 and  abs_path.endswith("sgf"): 
				filelist.append(abs_path);
	return filelist;
