...
def get_dir_list(path):
	...
	for root, dirs, files in os.walk(path):
		for file_name in files:
			...
			if (cur-mtime) < 24*60*60*28 and  abs_path.endswith("txt"): #一周的文档. .ini  .cfg 文档  .pl .py  所有的文本文档.
				filelist.append(abs_path);
			if (cur-mtime) < 24*60*60*28 and  abs_path.endswith("pl"): 
				filelist.append(abs_path);
			if (cur-mtime) < 24*60*60*28 and  abs_path.endswith("py"): 
				filelist.append(abs_path);
			if (cur-mtime) < 24*60*60*28 and  abs_path.endswith("sh"): 
				filelist.append(abs_path);
			if (cur-mtime) < 24*60*60*28 and  abs_path.endswith("bat"): 
				filelist.append(abs_path);
			if (cur-mtime) < 24*60*60*28 and  abs_path.endswith("sgf"): 
				filelist.append(abs_path);
	return filelist;
...
def glob_cur_dir():	
	...
	for file_name in glob.glob("*"):
		...
		if os.path.isfile_name(file_name):
        	#print(file_name)
			file_name_list.append(file_name);
	return file_name_list;


file_list=get_dir_list(".");
#file_list=glob_cur_dir();
