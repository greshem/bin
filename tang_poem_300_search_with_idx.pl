...
$number=shift or die("Usage: $0 idx_numbe\n");
if($number=~/\d+/)
{
	$number=sprintf("%03d", $number);

	load_tang_txt();
	my $poem_str=$g_tang_hash{$number};
	print  $poem_str;

		
	my @memory=get_memory_str($poem_str);
	print "cd /root/poem_hangzhi_memory/ && ./hanzhi2pingying.pl  ";
	print join(" ", @memory )."\n";
}
else
{
