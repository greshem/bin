...
dialog  --ok-label "Submit"           --help-button           --item-help           --backtitle "钱龙网卡设置"           \
--form "网卡设置." 20 50 		\
0         \
...
get_set_ip $eth  $ip $netmask $gateway $nameserver
if [ $? -eq 1 ];then
	dialog --msgbox "$eth 网卡设置成功" 10 20 
	echo ok$eth >> eth_setup
else
	dialog --msgbox "$eth 网卡设置失败" 10 20 
	return  1
fi
return 0
}
...
select_eth()
{
value="dialog --backtitle \"钱龙网卡设置\" \
	--title \"网卡设置\" \
	--default-item Dialog \
	--menu \"请选择网卡并进行设置 \" 19 50 6 "
